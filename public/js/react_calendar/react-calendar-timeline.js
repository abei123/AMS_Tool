...
/***/ function(module, exports, __webpack_require__) {
	...
	var Item = function (_Component) {
	  ...
	  _createClass(Item, [{
	    ...
	    value: function render() {
          ...
          var showLeftShadow = (this.props.item.itemProps.hasLeftShadow != 'false');
          var showRightShadow = (this.props.item.itemProps.hasRightShadow != 'false');
	      ...
	      var classNames = 'rct-item' + (this.props.selected ? ' selected' : '') + (this.canMove(this.props) ? ' can-move' : '') + (this.canResizeLeft(this.props) || this.canResizeRight(this.props) ? ' can-resize' : '') + (this.canResizeLeft(this.props) ? ' can-resize-left' : '') + (this.canResizeRight(this.props) ? ' can-resize-right' : '') + (this.props.item.className ? ' ' + this.props.item.className : '') + (dimensions.clippedLeft ? ' clipped-left' : '') + (dimensions.clippedRight ? ' clipped-right' : '');
				...
				var boxColor = '#000000';
				switch (this.props.item.className ) {
					case 'item-normal':
						boxColor = '#90D3E6'
						break;
					case 'item-bad':
	      ...
	      var shadowLeft = '' + dimensions.shadowLeft;
	      var shadowRight = '' + dimensions.shadowRight;

	      var boxShadow = false;
            var boxShadowL = false;
            var boxShadowR = false;
	      var style;

	      if ((showLeftShadow) && (shadowLeft < '' + dimensions.left)) {
              boxShadowL = shadowLeft - ('' + dimensions.left) + 'px 0px 0px #ff6868';
	      }

          if ((showRightShadow) && (shadowRight)) {
              boxShadowR = shadowRight + 'px 0px 0px' + boxColor;
	      }

          if (boxShadowL) {
              boxShadow = boxShadowL;
          }
          if (boxShadowL && boxShadowR) {
              boxShadow = boxShadow + ',' + boxShadowR;
          }
          if (!boxShadowL && boxShadowR) {
              boxShadow = boxShadowR;
          }
		  ...
		  var style_tooltip = {
			left: (dimensions.left+dimensions.width-30) + 'px'
			,top: (dimensions.top+5) + 'px'
			};

	      var origin = this.props.item.itemProps.flightOrigin;
	      var destination = this.props.item.itemProps.flightDestination;
	      var arrivalTime = this.props.item.itemProps.arrivalTime;
	      var departTime = this.props.item.itemProps.departTime;
	      var notifs = this.props.item.itemProps.notifications;
	      var notifications = void 0;

	      if (notifs && notifs.length > 0) {

	        notifications = notifs.map(function (notificationItem, id) {
	          return _react2.default.createElement('img', { key: id, src: '/img/' + notificationItem + '.png' });
	        });
	      }

	      return _react2.default.createElement('div', {className:'tooltip-wrapper'},
      	createTooltip(style_tooltip, this.itemTitle, origin, destination, arrivalTime, departTime, this.props.item.itemProps.flightObject),
		_react2.default.createElement(
	        'div',
	        ...
	        _react2.default.createElement(
	          'div',
	          { className: 'rct-item-overflow item-split-pane' },
	          _react2.default.createElement(
	            'div',
	            { className: 'item-split-pane-left' },
	            _react2.default.createElement(
	              'div',
	              { className: 'item-port-name' },
	              origin
	            )
	          ),
	          _react2.default.createElement(
	            'div',
	            { className: 'item-split-pane-left-details' },
	            '\xA0',
	            departTime
	          ),
	          _react2.default.createElement(
	            'div',
	            { className: 'rct-item-content item-split-pane-center' },
	            this.itemTitle
	          ),
	          _react2.default.createElement(
	            'div',
	            { className: 'item-split-pane-right' },
	            _react2.default.createElement(
	              'div',
	              { className: 'item-port-name' },
	              destination
	          ...
	          _react2.default.createElement(
	            'div',
	            { className: 'item-split-pane-right-details' },
	            arrivalTime,
	            '\xA0',
	            _react2.default.createElement(
	              'div',
	              { className: 'item-split-pane-right-icons' },
	              notifications
	            )
	          )
	        ),
	...
	function createTooltip(style_tooltip, vfltNum, origin, destination, arrivalTime, departTime, fltObj ){
		//set times for display
		var prjDep = new Date(fltObj.prjGMTDepTime);
		var prjDepTime = getTimeHHMM(prjDep);
		var prjDepDt = getDateMMDDYYYY(prjDep);

		var prjArr = new Date(fltObj.prjGMTArrTime);
		var prjArrTime = getTimeHHMM(prjArr);
		var prjArrDt = getDateMMDDYYYY(prjArr);

		var schDep = new Date(fltObj.schGMTDepTime);
		var schDepTime = getTimeHHMM(schDep);
		var schDepDt = getDateMMDDYYYY(schDep);

		var schArr = new Date(fltObj.schGMTArrTime);
		var schArrTime = getTimeHHMM(schArr);
		var schArrDt = getDateMMDDYYYY(schArr);

		var alertDiv = formAlertDiv(fltObj);


		return _react2.default.createElement(
			'div',
			{className: 'tooltip-view', style: style_tooltip},
			_react2.default.createElement(
				'div', {className: 'tooltip-header'}
				, _react2.default.createElement('img', {src:'img/logo_small.png'} )
				,vfltNum
			)
			, _react2.default.createElement(
				'div'
				...
				, _react2.default.createElement(
					...
					, _react2.default.createElement(
						...
						, _react2.default.createElement(
							'tr',{}
							, _react2.default.createElement(
								'td',{className:'tooltip-header2', colspan:3}
								,origin
								, _react2.default.createElement('img', {src:'img/hover_plane_grey2.png'} )
								,destination
							)
						)
						, _react2.default.createElement(
							'tr', {}
							, _react2.default.createElement('td', {className:'tooltip-label'}, 'Scheduled Time')
							, _react2.default.createElement('td', {className:'tooltip-data'}, schDepTime+' - '+schArrTime)
							, _react2.default.createElement('td', {className:'tooltip-data'}, schDepDt)
						)
						, _react2.default.createElement(
							'tr', {}
							, _react2.default.createElement('td', {className:'tooltip-label'}, 'Projected Time')
							, _react2.default.createElement('td', {className:'tooltip-data'}, prjDepTime+' - '+prjArrTime)
							, _react2.default.createElement('td', {className:'tooltip-data'}, prjDepDt)
						)
					)
				) //***fltinfo table
				, alertDiv
	...
	function getTimeHHMM(dateVal){
		return prependZero(dateVal.getHours(),2) + ":" + prependZero(dateVal.getMinutes(),2);
	}
	function getDateMMDDYYYY(dateVal){
		return prependZero((dateVal.getMonth()+1),2) + "/" + prependZero(dateVal.getDate(),2) + "/" + dateVal.getFullYear();
	}
	function prependZero(value, expectedLength){
		var s = value+"";
		while(s.length<expectedLength){	s = "0" + s; }
		return s;
	}
	function formAlertDiv(fltObj){
		var alertMetalSwap = createAlertMetalSwap(fltObj.schEqpCode, fltObj.asgEqpCode);
		var alertGroundTime = createGroundTimeAlert(fltObj.groundTime, 20);	//TODO: do not hardcode 20! check if not fixed at 20
		var alertCrewChange = null;
		var alertOthers = createOtherAlerts(fltObj.prbTypes);

		if(alertMetalSwap!=null || alertGroundTime!=null || alertCrewChange!=null || alertOthers!=null){
			return _react2.default.createElement(
				'div', {className:'tooltip-alert'}
				, _react2.default.createElement('hr', {})
				, _react2.default.createElement('div', {className:'tooltip-alert-header'}, 'ALERTS')
				, alertMetalSwap==null ? '' : alertMetalSwap
				, alertGroundTime==null ? '' : alertGroundTime
				, alertCrewChange==null ? '' : alertCrewChange
				, alertOthers==null ? '' : alertOthers
			);
		}else {
			return '';
		}
	}
	function createAlertMetalSwap(asgEqpCode, schEqpCode){
		//check metal swap
		if(schEqpCode == asgEqpCode){
			return null;
		} else {
			return _react2.default.createElement(
				...
				,'Metal Swap'
				, _react2.default.createElement(
					'table', {className: 'tooltip-metalswap'}
					, _react2.default.createElement(
						'tbody', {}
						, _react2.default.createElement(
							'tr', {}
							, _react2.default.createElement('td', {className:'tooltip-label'}, 'Scheduled Equipment')
							, _react2.default.createElement('td', {className:'tooltip-data'}, schEqpCode)
						)
						, _react2.default.createElement(
							'tr', {}
							, _react2.default.createElement('td', {className:'tooltip-label'}, 'Assigned Equipment')
							, _react2.default.createElement('td', {className:'tooltip-data'}, asgEqpCode)
					  )
				  )
			  )
		  );
	...
	function createGroundTimeAlert(groundTime, minGroundTime){
		if(groundTime>=minGroundTime){
			return null;
		} else {
			return _react2.default.createElement(
				'div', {className: 'tooltip-data'}, 'Ground less than '+minGroundTime+' minutes ('+groundTime+')'
			);
		}
	}
	function createOtherAlerts(problems){
		problems = jQuery.grep(problems, function(value) {
  		//return value != 'REGULAR';
			return value.indexOf("REGULAR")<0
		});
...
/***/ function(module, exports, __webpack_require__) {
	...
	function calculateDimensions(_ref) {
	  ...
	  var shadowStart = item.itemProps.plannedDeparture;
	  var shadowEnd = item.itemProps.plannedArrival;

	  var x = isDragging ? dragTime : itemStart;
	  var shadowX = item.itemProps.plannedDeparture;

	  var w = Math.max(itemEnd - itemStart, dragSnap);
	  var shadowW = Math.max(item.itemProps.plannedArrival - itemEnd, dragSnap);
	  ...
	  var dimensions = {
	    ...
	    shadowLeft: (shadowX - canvasTimeStart) * ratio,
	    top: null,
	    width: Math.max(w * ratio, 3),
	    shadowRight: Math.max(shadowW * ratio, 3),
	    height: h,
	...
	var Header = function (_Component) {
	  ...
	  _createClass(Header, [{
	    ...
	    value: function headerLabel(time, unit, width) {
	      if (unit === 'year') {
	        return time.utcOffset(props.customUtcOffset).format(width < 46 ? 'YY' : 'YYYY');
	      } else if (unit === 'month') {
	        return time.utcOffset(props.customUtcOffset).format(width < 65 ? 'MM/YY' : width < 75 ? 'MM/YYYY' : width < 120 ? 'MMM YYYY' : 'MMMM YYYY');
	      } else if (unit === 'day') {
	        return time.utcOffset(props.customUtcOffset).format(width < 150 ? 'L' : 'dddd, LL');
	      } else if (unit === 'hour') {
	        return time.utcOffset(props.customUtcOffset).format(width < 50 ? 'HH' : width < 130 ? 'HH:00' : width < 150 ? 'L, HH:00' : 'dddd, LL, HH:00');
	      } else {
	        return time.utcOffset(props.customUtcOffset).format('LLL');
	      }
	    }
	  }, {
	    key: 'subHeaderLabel',
	    value: function subHeaderLabel(time, unit, width) {
	      if (unit === 'year') {
	        ...
	        return time.utcOffset(props.customUtcOffset).format(width < 37 ? 'MM' : width < 85 ? 'MMM' : 'MMMM');
	      } else if (unit === 'day') {
	        return time.utcOffset(props.customUtcOffset).format(width < 47 ? 'D' : width < 80 ? 'dd D' : width < 120 ? 'ddd, Do' : 'dddd, Do');
	      } else if (unit === 'hour') {
	        return time.utcOffset(props.customUtcOffset).format(width < 50 ? 'HH' : 'HH:00');
	      } else if (unit === 'minute') {
	        return time.utcOffset(props.customUtcOffset).format(width < 60 ? 'mm' : 'HH:mm');
	      } else {
	        return time.utcOffset(props.customUtcOffset).get(unit);
	      }
	    }
	  }, {
	    key: 'resetTouchState',
